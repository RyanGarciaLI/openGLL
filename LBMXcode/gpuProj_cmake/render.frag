...
void main()
{
    ...
    vec2 radiusVec = vec2(10, 10) / image_size;
     if (int(pos.x * image_size.x) % 2 == 0 && int(pos.y * image_size.y) % 2 == 0)
//    if (dist <= length(radiusVec))
    {
        FragColor = vec4( 1.0f, 0.0f, 0.0f, 0.0f );
    }
    else //if (texture(boundary_texture, pos).x > -1.0)
    {
        // fluid
...
//        }
//        FragColor = vec4( color*0.4, color*0.6, color, 0.0 );
        FragColor = texture(state_texture2, pos).z * vec4(1.0, 1.0, 1.0, 0.0);
    }
//    else
//    {    // boundaries
